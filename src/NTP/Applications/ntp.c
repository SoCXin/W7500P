...
uint8_t NTP_Server_IP[4]={202, 112, 10, 60}; /*NTP服务器——北京邮电大学*/
...
*@return		无
*/
uint8_t NTP_Retry_Cnt=0; 													/*请求重传次数*/
void do_ntp_client(void)
{
  if(Total_Seconds)																/*已获取时间则不再执行NPT程序*/
    return; 
  else
  {
    uint16_t len;
    ...
    uint32_t destip = 0;
    uint16_t destport;
    uint16_t startindex = 40; 										/*回复包中时间数据首地址*/
    switch(getSn_SR(SOCK_NTP))
    {
      case SOCK_UDP:															/*UDP模式开启*/
        ...
        {
          if (len > TX_RX_MAX_BUF_SIZE) len = TX_RX_MAX_BUF_SIZE;	/*接收NTP服务器回复数据*/
          recvfrom(SOCK_NTP, data_buf, len, (uint8_t*)&destip, &destport);	/*从NTP服务器获取时间*/
          //printf("%d\r\n", data_buf);
        ...
        break;
      case SOCK_CLOSED:
        socket(SOCK_NTP,Sn_MR_UDP,NTP_Port,0x01);//标志位flag不能为0，否则有错误
        break;
    }
  } 
}
