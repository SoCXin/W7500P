...
//				*value= HTONL(snmpData[id].u.intval);
			temp= HTONL(snmpData[id].u.intval);
			((uint8_t*)value)[0] = (uint8_t)(temp>>24);
			((uint8_t*)value)[1] = (uint8_t)((temp&0x00ff0000)>>16);
			((uint8_t*)value)[2] = (uint8_t)((temp&0x0000ff00)>>8);
			((uint8_t*)value)[3] = (uint8_t)(temp&0x000000ff);			
		}
	break;
	default : 
...
int32 makeTrapVariableBindings(dataEntryType *oid_data, void *ptr, uint32 *len)
{
	...
	//			*(int32*)(&((uint8*)ptr)[4+oid_data->oidlen+2]) = HTONL(oid_data->u.intval);
				 temp=HTONL(oid_data->u.intval);
				
				((uint8_t*)ptr)[4+oid_data->oidlen+2] = (uint8_t)(temp>>24);
				((uint8_t*)ptr)[4+oid_data->oidlen+3] = (uint8_t)((temp&0x00ff0000)>>16);
				((uint8_t*)ptr)[4+oid_data->oidlen+4] = (uint8_t)((temp&0x0000ff00)>>8);
				((uint8_t*)ptr)[4+oid_data->oidlen+5] = (uint8_t)(temp&0x000000ff);
